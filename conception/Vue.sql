...
FROM Trajet t
    ...
    AND tr.id_reservation_statut NOT IN (
        SELECT id FROM Reservation_Status WHERE libelle = 'Annulé'
    )
...
FROM Trajet_Reservation_Mouvement_Status trms
         INNER JOIN Reservation_Status rs ON trms.id_nouveau_statut = rs.id
...
FROM Trajet_Reservation tr
         ...
         INNER JOIN Reservation_Status rs ON tr.id_reservation_statut = rs.id
