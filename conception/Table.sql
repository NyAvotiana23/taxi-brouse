...
-- ============================================
-- 11. GESTION DES UNITÉS ET CONFIGURATION
-- ============================================
CREATE TABLE Unite
(
    id      BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX idx_configuration_code ON Configuration (code);

-- ============================================
-- 12. GESTION DES PUBLICITÉS ET DIFFUSIONS
-- ============================================
...
CREATE INDEX idx_publicite_societe ON Publicite (id_societe);
...
(
    ...
    id_trajet    BIGINT         NOT NULL,
    montant_unite DECIMAL(15, 2) NOT NULL CHECK (montant_unite >= 0),
    nombre       INTEGER        NOT NULL CHECK (nombre > 0),
    FOREIGN KEY (id_publicite) REFERENCES Publicite (id),
    FOREIGN KEY (id_trajet) REFERENCES Trajet (id)
);

CREATE INDEX idx_diffusion_publicite ON Diffusion (id_publicite);
CREATE INDEX idx_diffusion_trajet ON Diffusion (id_trajet);

CREATE TABLE Diffusion_Paiement
(
    id             BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_diffusion   BIGINT         NOT NULL,
    id_societe     BIGINT         NOT NULL,
    date_paiement  TIMESTAMP      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    montant        DECIMAL(15, 2) NOT NULL CHECK (montant > 0),
    FOREIGN KEY (id_diffusion) REFERENCES Diffusion (id),
    FOREIGN KEY (id_societe) REFERENCES Societe (id)
);

CREATE INDEX idx_diffusion_paiement_diffusion ON Diffusion_Paiement (id_diffusion);
CREATE INDEX idx_diffusion_paiement_societe ON Diffusion_Paiement (id_societe);